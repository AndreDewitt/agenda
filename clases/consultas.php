<?php
  ...
  class consultas extends Conexion {
    ...
    function insertarUsuario($datos) {
      $conexion = Conexion::Conectar();
      $password = sha1($datos[1]);
      if($this->usuarioRepetido($datos[0]) == 1) {
        return 11;
      } else {
        $sql = "INSERT INTO t_usuarios(nombre_usuario,password,foto_usuario,nombre,apellidos,fecha_nacimiento,
          edad,telefono,email) VALUES('$datos[0]','$password','$datos[8]','$datos[2]','$datos[3]','$datos[4]','$datos[5]','$datos[6]','$datos[7]')";
        $resul = mysqli_query($conexion,$sql);
        if ($resul) {
          return 1;
        } else {
    ...
    public function usuarioRepetido($nom) {
      $conexion = Conexion::Conectar();
      $sql = "SELECT nombre_usuario FROM t_usuarios WHERE nombre_usuario = '$nom'";
      $resul = mysqli_query($conexion,$sql);
    ...
    function registrarCategorias($datos) {
      ...
      $resul1 = mysqli_query($conexion,$sql1);
      $id = mysqli_fetch_array($resul1);
      if ($this->categoriaRepetida($datos[0],$id[0])==1) {
        return 11;
      } else {
    ...
    public function categoriaRepetida($nom,$iduser) {
      $conexion = Conexion::Conectar();
      $sql = "SELECT nombre_categoria FROM t_categorias WHERE nombre_categoria = '$nom' AND id_usuario = '$iduser'";
      $resul = mysqli_query($conexion,$sql);
      if (mysqli_num_rows($resul)>0) {
        return 1;
      } else {
